...
def view_layers(x, input_shape, trainable=False):
    ...
    x = TimeDistributed(AveragePooling2D((7, 7)), name='avg_pool_view')(x)
...
def classifier(base_layers, input_rois, num_rois, nb_classes = 21, trainable_cls= True ,trainable_view = True):
    ...
    out1 = view_layers(out_roi_pool, input_shape=input_shape, trainable=trainable_view)
    ...
    out_view = TimeDistributed(Dense(360 * nb_classes, activation='linear', kernel_initializer='glorot_normal',trainable=trainable_view), name='dense_view_{}'.format(nb_classes))(out1)
    ...
    return [out_class, out_regr, out_view]
