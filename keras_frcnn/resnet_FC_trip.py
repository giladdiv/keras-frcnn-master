...
def view_layers(x, input_shape, trainable=False):
    ...
    x2 = TimeDistributed(Convolution2D(512, (1, 1), trainable= trainable, kernel_initializer='normal',padding='same'), name='bla_bla')(x)
...
def classifier(base_layers, input_rois, num_rois, nb_classes = 21, trainable_cls= True ,trainable_view = True):
    ...
    ##use FC
    out1_temp = TimeDistributed(Dense(1024, activation='linear', kernel_initializer='glorot_normal',trainable=True), name='dense_fc1024_view_{}'.format(nb_classes))(fc)
    ...
    out1 = TimeDistributed(Activation('relu'))(out1_temp)
    ...
    return [out_class, out_regr, out_view],out1_temp
