...
def calc_rpn_gilad(C, bbox, width, height, resized_width, resized_height):
	...
	# get the GT box coordinates, and resize to account for image resizing
	gta[0, 0] = int(bbox['x1']) * (resized_width / float(width))
	gta[0, 1] = int(bbox['x2']) * (resized_width / float(width))
	gta[0, 2] = int(bbox['y1']) * (resized_height / float(height))
	gta[0, 3] = int(bbox['y2']) * (resized_height / float(height))
				...
				for jy in range(output_height):
					...
					for bbox_num in range(num_bboxes):
						...
						if img_data['bboxes'][bbox_num]['class'] != 'bg':
							...
							# all GT boxes should be mapped to an anchor box, so we keep track of which anchor box was bestcalc_rpn
							if curr_iou > best_iou_for_bbox[bbox_num]:
								best_anchor_for_bbox[bbox_num] = [jy, ix, anchor_ratio_idx, anchor_size_idx]
								best_iou_for_bbox[bbox_num] = curr_iou
								best_x_for_bbox[bbox_num,:] = [x1_anc, x2_anc, y1_anc, y2_anc]
...
def get_anchor_gt(all_img_data, class_count, C, backend, mode='train',create_flip = False):
	...
	while True:
		...
		for img_data in all_img_data:
			try:
				...
				# 	print ('didnt use image in data generator line 475' )
				# 	continue
				...
				if create_flip:
					...
					x_img_flip[:, :, 0] -= C.img_channel_mean[0]
					x_img_flip[:, :, 1] -= C.img_channel_mean[1]
					x_img_flip[:, :, 2] -= C.img_channel_mean[2]
...
def get_anchor_gt_trip(img_data, C, backend):
	"""
	prepare data for triplet loss
	:param img_data:
	:param C:
	:param backend:
	:return:
...
def prep_img_siam(img,C, backend, mode='train'):
	try:
		# read in image, and optionally add augmentation
