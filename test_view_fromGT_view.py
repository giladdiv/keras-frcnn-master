...
weight_path = '/home/gilad/ssd/keras-frcnn-master/model_frcnn_tmp.hdf5'
...
classifier_only = nn.classifier(feature_map_input, roi_input, C.num_rois, nb_classes=21, trainable=True)
...
for im_file in all_imgs:
	...
	for bbox_gt in im_file['bboxes']:
		...
		while no_bbox_flag and bbox_threshold > th_bbox:
			...
			# y_rpn_cls, y_rpn_regr= data_generators.calc_rpn_gilad(C, bbox_gt, width=width,height=height, resized_width=resized_width, resized_height=resized_height)
			# y_rpn_regr[:, y_rpn_regr.shape[1] // 2:, :, :] *= C.std_scaling## read the training data from pickle file or from annotations
			...
			# R = roi_helpers.rpn_to_roi(y_rpn_cls, y_rpn_regr, C, K.image_dim_ordering(), overlap_thresh=0.7)
			#
			# # convert from (x1,y1,x2,y2) to (x,y,w,h)
			# R[:, 2] -= R[:, 0]
			# R[:, 3] -= R[:, 1]
