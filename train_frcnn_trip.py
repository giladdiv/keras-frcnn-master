...
parser.add_option("--output_weight_path", dest="output_weight_path", help="Output path for weights.", default='./model_fix_base.hdf5')
...
parser.add_option("--input_train_file", dest="input_train_file", help="if there is a pickle file for train data.",default='./train_data_only_Wflip_pascal.pickle' )
...
## siam network part
C.siam_iter_frequancy = 101
weight_path_init = os.path.join(base_path, 'model_frcnn_new.hdf5')
...
def build_models(weight_path,init_models = False,train_view_only = False,create_siam_trip = False):
	##
	if train_view_only:
		trainable_cls = False
		trainable_view = True
...
_,_,_,model_view_only,model_siam,model_flip = build_models(weight_path = weight_path_tmp, init_models = True,create_siam_trip=True, train_view_only = True)
## get layers
	...
	while True:
		...
		try:
			...
			if  siam_flip and iter_num % C.siam_iter_frequancy == 0 :
				...
				model_flip.train_on_batch([X, R_orig,X_flip,R_flip,Y_orig],np.array([[1]]))
				loss_after = model_flip.predict([X, R_orig,X_flip,R_flip,Y_orig])
