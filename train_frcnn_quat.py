...
from keras_frcnn import config_64, data_generators
from keras_frcnn import losses as losses
from keras_frcnn import resnet_quat as nn
import keras_frcnn.roi_helpers as roi_helpers
from keras.utils import generic_utils
from test_view_func_quat import *
...
parser.add_option("--output_weight_path", dest="output_weight_path", help="Output path for weights.", default='models/model_quat.hdf5')
...
if os.path.exists(options.input_train_file):
	with open(options.input_train_file) as f:
		...
		if data_type == 'real':
			...
			all_imgs_temp = pickle.load(f)
			classes_count ={}
			for key,items in all_imgs_temp.items():
				for item in items:
...
classifier,_ = nn.classifier(shared_layers, roi_input, C.num_rois, nb_classes=len(classes_count),trainable_cls=False,trainable_view = True)
...
try:
	model_rpn.load_weights(init_weights,by_name=True)
	model_classifier.load_weights(init_weights,by_name=True)
...
model_classifier.compile(optimizer=optimizer_classifier, loss=[losses.class_loss_cls, losses.class_loss_regr(len(classes_count)-1),losses.quat_loss(len(classes_count),roi_num=C.num_rois)], metrics={'dense_class_{}'.format(len(classes_count)): 'accuracy'})
...
for epoch_num in range(num_epochs):
	...
	while True:
		...
		try:
			...
			numNonBg = C.num_rois-np.sum(np.argmax(Y_view[:, sel_samples, :],axis=2)==20)
			...
			if iter_num == epoch_length:
				...
				if curr_loss < best_loss:
					...
					# cmd = "python {} --ep {} --weight {}".format(os.path.join(base_path,'test_view_fromGT.py'),epoch_num,C.model_path[2:])
					# K.clear_session()
					# os.system(cmd)
				...
				if epoch_num%epoch_save_num== 0 and epoch_num !=0:
					...
					tmp_succ,mAP= test_view_func_quat(C, model_rpn, model_classifier)
