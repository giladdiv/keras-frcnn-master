...
def loss_angular(y_true,y_pred):
	a = 36 * np.pi/180
	x_a = y_pred[0]
	x_p = y_pred[1]
	x_n = y_pred[2]
	x_c = 0.5*(x_a+x_p)
	e_ap = K.mean(K.sum(K.square(x_a - x_p), axis=2),axis=1)
	e_nc = K.mean(K.sum(K.square(x_n - x_c), axis=2),axis=1)
	return K.maximum(e_ap - 4*np.tan(a)**2 * e_nc,K.epsilon())
...
def build_models(weight_path,init_models = False,train_view_only = False,create_siam = False):
	...
	if create_siam:
		...
		## flat the layers
		# view_ref = Flatten()(inner_ref)
		# view_dp = Flatten()(inner_dp)
		# view_dm = Flatten()(inner_dm)
        #
		...
		# dist = Lambda(lambda x: x * soft_param)(dist)
		# trip = Activation('softmax')(dist)  # should be comperd to [0,1] becase dp shold be small and dm large so after softmax it
		model_trip = Model([img_input_ref, roi_input_ref, img_input_dp, roi_input_dp, img_input_dm, roi_input_dm,labels_input],[inner_ref,inner_dp,inner_dm])
		# model_trip.layers[10].trainable_weights.extend([soft_param])
		model_trip.compile(optimizer=optimizer_trip, loss=loss_angular)
...
## get layers
# succ_tmp,mm =test_view_func(C, model_rpn, model_classifier)
# print('succ {}'.format(succ_tmp))
# print('sum {}'.format(sum(succ_tmp)))
# classifier_not_trainable_layers = [i for i,x in enumerate(model_classifier.layers) if x.trainable == False]
...
# func_k = K.function([img_input_ref, roi_input_ref, img_input_dp, roi_input_dp, img_input_dm, roi_input_dm,labels_input],[model_trip.layers[10].input,model_trip.layers[10].output])
